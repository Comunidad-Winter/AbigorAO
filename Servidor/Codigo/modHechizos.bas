...
Dim Masc As Integer
   ...
   If UserList(UserIndex).POS.Map = 60 Then
    Call SendData(ToIndex, UserIndex, 0, "||No puedes invocar criaturas en este mapa!" & FONTTYPE_FENIX)
    Exit Sub
    End If
...
Amigos = (((UserList(UserIndex).Faccion.Bando = UserList(UI).Faccion.Bando) Or (EsNewbie(UI)) Or (EsNewbie(UserIndex)))) Or (UserList(UserIndex).POS.Map = 60) Or (UserList(UserIndex).Faccion.Bando = Neutral)
...
If UserList(UserIndex).POS.Map = 60 Then
Call SendData(ToIndex, UserIndex, 0, "||No puedes envenenar a nadie en este mapa" & FONTTYPE_FENIX)
Exit Sub
End If
End If
...
If UserList(UserIndex).POS.Map = 60 Then
Call SendData(ToIndex, UserIndex, 0, "||No puedes revivir a nadie en este mapa" & FONTTYPE_FENIX)
Exit Sub
End If
End If
...
If Hechizos(H).Revivir = 1 Then
    If UserList(TU).flags.Muerto Then
        Call SendData(ToIndex, UserIndex, 0, "REV")
        Revividor = TU
        Call InfoHechizo(UserIndex)
        B = True
        Exit Sub
...
ElseIf Hechizos(hIndex).SubeHP = 2 Then
    ...
    Daño = DañoHechizo(UserIndex, hIndex)
    If ObjData(UserList(UserIndex).Invent.WeaponEqpObjIndex).Baculo = 5 Then
    Daño = 1.75 * Daño
    End If
    If ObjData(UserList(UserIndex).Invent.WeaponEqpObjIndex).Baculo = 4 Then
    Daño = 1.35 * Daño
    End If
    If ObjData(UserList(UserIndex).Invent.WeaponEqpObjIndex).Baculo = 3 Then
    Daño = 1.15 * Daño
    End If
    If ObjData(UserList(UserIndex).Invent.WeaponEqpObjIndex).Baculo = 2 Then
    Daño = 1.05 * Daño
    End If
...
+ 10 * Buleano(UserList(UserIndex).Clase = MAGO And UserList(UserIndex).Recompensas(3) = 2 And Hechizo = 35), _
...
+ 20 * Buleano(UserList(UserIndex).Clase = CLERIGO And UserList(UserIndex).Recompensas(3) = 1 And Hechizo = 5) _
+ 10 * Buleano(UserList(UserIndex).Clase = MAGO And UserList(UserIndex).Recompensas(3) = 1 And Hechizo = 9))
...
If Hechizos(H).Nombre = "Portal Luminoso" Then
    If UserList(UserIndex).POS.Map = 62 Then Exit Sub 'zona espera
    If UserList(UserIndex).POS.Map = 60 Then Exit Sub 'zonas de "torneo"
    If UserList(UserIndex).POS.Map = 37 Then Exit Sub 'Carcel
...
If Hechizos(H).Materializa = 1 Then 'matute
    ...
    Dim ET As Obj
    ET.Amount = 1
    ET.OBJIndex = 840 'veamos asd - [Primer FX que se ve en la imagen 1] -VER OBJ.DAT
