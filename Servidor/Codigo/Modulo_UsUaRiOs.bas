...
If UserList(AttackerIndex).POS.Map <> 60 Then
...
Call SendData(ToIndex, AttackerIndex, 0, "||Has matado al mas buscado. Tu recompenza fue Una espada mata dragones." & FONTTYPE_GUILD)
Call SendData(ToAll, 0, 0, "Atencion!!: " & UserList(VictimIndex).Name & " ha eliminado a " & UserList(VictimIndex).Name & ", (el mas buscado)." & FONTTYPE_INFO)
ElMasBuscado = 0
End If
...
ElseIf UserList(UserIndex).GuildInfo.FundoClan = 1 Then
    bCr = 6
ElseIf UserList(UserIndex).flags.VIP = 1 And UserList(UserIndex).flags.Privilegios = 0 Then
    bCr = 7
Else: bCr = 5
End If
...
ElseIf UserList(UserIndex).GuildInfo.FundoClan = 1 Then
    bCr = 6
ElseIf UserList(UserIndex).flags.VIP = 1 Then
    bCr = 7
Else
    bCr = 5
End If
...
If UserList(UserIndex).Stats.Exp >= UserList(UserIndex).Stats.ELU Then
    ...
    Dim infer As String
infer = UserList(UserIndex).Char.CharIndex
Call SendData(ToIndex, UserIndex, UserList(UserIndex).POS.Map, "||" & vbGreen & "°" & "Vida +" & _
AumentoHP & " Mana +" & AumentoMANA & " Golpe +" & SubeHit & " Energia +" & AumentoST & " Skills +" & Pts & "." & "°" & infer)
    ...
    If Not EsNewbie(UserIndex) And WasNewbie Then
        If UserList(UserIndex).POS.Map = 41 Or UserList(UserIndex).POS.Map = 42 Then
            Call WarpUserChar(UserIndex, 1, 50, 69, True)
        Else
...
Dim email As Long
    ...
    Call SendData(ToIndex, Ganador, 0, "||Utiliza el comando /cambiaremail para poner configurar el mail a tu dirección" & FONTTYPE_TALK)
        ...
        UserList(UserIndex).email = tuemail
...
If UserList(UserIndex).POS.Map <> 60 Then UserList(UserIndex).Stats.VecesMurioUsuario = UserList(UserIndex).Stats.VecesMurioUsuario + 1
...
Call SendUserStatsBox(UserIndex)
        ...
        dMoney = ""
       Call WarpUserChar(uDuelo1, 1, 50, 69, True)
       Call WarpUserChar(uDuelo2, 1, 50, 79, True)
    End If
...
If UserList(Muerto).POS.Map = 60 Then Exit Sub
...
Sub Cerrar_Usuario(UserIndex As Integer)
        ...
        dMoney = ""
       Call WarpUserChar(uDuelo1, 1, 50, 69, True)
       Call WarpUserChar(uDuelo2, 1, 50, 70, True)
End If
...
Call SendData(ToAll, 0, 0, "Atencion!!: El más buscado se ha desconectado." & FONTTYPE_GUILD)
ElMasBuscado = 0
End If
