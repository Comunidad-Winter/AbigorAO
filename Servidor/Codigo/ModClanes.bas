...
If Not TieneObjetos(gemaliderazgo.OBJIndex, 1, UserIndex) Then
    Call SendData(ToIndex, UserIndex, 0, "||Necesitas tener la Gema del Liderazgo para poder fundar clan" & FONTTYPE_INFO)
    Exit Function
End If
...
If Not UserList(UserIndex).Clase = PIRATA Then
    Call SendData(ToIndex, UserIndex, 0, "||Debes ser Pirata para poder fundar un clan." & FONTTYPE_INFO)
    Exit Function
End If
...
CreateGuild = True
Call QuitarObjetos(412, 1, Index)
Call SaveGuildsNew
