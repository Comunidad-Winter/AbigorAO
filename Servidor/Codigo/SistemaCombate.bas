...
If UserList(UserIndex).flags.Invisible = 1 Then
    Call SendData(ToIndex, UserIndex, 0, "||Has vuelto a ser visible por intentar atacar a otro usuario." & FONTTYPE_INFO)
...
Else
     ...
     Call SendData(ToPCArea, UserIndex, UserList(UserIndex).POS.Map, "||" & vbRed & "°Fallas°" & str(UserList(UserIndex).Char.CharIndex))
End If
...
If UserList(AttackerIndex).flags.Invisible = 1 Then
    Call SendData(ToIndex, AttackerIndex, 0, "||Has vuelto a ser visible por intentar atacar a otro usuario." & FONTTYPE_INFO)
    Call QuitarInvisible(AttackerIndex)
    Exit Function
End If
...
If UserList(VictimIndex).POS.Map = 62 Then
...
If UserList(AttackerIndex).Faccion.Bando <> Neutral And UserList(AttackerIndex).Faccion.Bando = UserList(VictimIndex).Faccion.Bando And UserList(AttackerIndex).POS.Map <> 60 And UserList(AttackerIndex).POS.Map <> 60 Then
