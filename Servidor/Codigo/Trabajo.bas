...
If UserList(UserIndex).Clase = MINERO Then
    MiObj.Amount = Fix(100 + ((0.29 + 0.07 * Buleano(UserList(UserIndex).Recompensas(1) = 1 And UserList(UserIndex).Invent.HerramientaEqpObjIndex = PICO_EXPERTO)) * UserList(UserIndex).Stats.UserSkills(Mineria)))
Else: MiObj.Amount = 1
End If
