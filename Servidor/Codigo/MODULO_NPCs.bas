...
Call ReSpawnNpc(MiNPC)
         ...
         If MiNPC.GiveGLD > 0 Then
            Call SendData(ToIndex, UserIndex, 0, "||Has ganado " & PonerPuntos(MiNPC.GiveGLD) & " monedas de oro al matar a la criatura." & FONTTYPE_FIGHT)
        Else
            Call SendData(ToIndex, UserIndex, 0, "||La criatura no ha dejado oro." & FONTTYPE_FIGHT)
        End If
...
If S >= 0 Then
    ...
    Npclist(NpcIndex).GiveEXP = INIDarClaveInt(A, S, "GiveEXP") * 1000
    ...
    Npclist(NpcIndex).GiveGLD = INIDarClaveInt(A, S, "GiveGLD") * 65
